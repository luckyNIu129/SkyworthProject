...
(function($, window, document, undefined) {
	...
	$.extend(Plugin.prototype, {
		...
		viewHtml: function() {
			...
			if (settings.showPN) {
				settings.page == 1 ? this.pageList.push('<button class="button-disabled" data-page="prev">' + settings.prevPage +
						'</button>') :
			...
			if (pageTatol <= 6) {
				...
				if (settings.page < 5) {
					...
					this.pageList.push('<button data-page="after" class="spage-after">...</button><button data-page=' + pageTatol +
						'>' + pageTatol + '</button>');
			...
			if (settings.showPN) {
				settings.page == pageTatol ? this.pageList.push('<button class="button-disabled" data-page="next">' + settings.nextPage +
						'</button>') :
			...
			if (settings.showSkip) {
				pageArr.push('<div class="spage-skip">跳至&nbsp;<input type="text" value="' + settings.page +
					'"/>&nbsp;页&nbsp;&nbsp;<button data-page="go">确定</button></div>');
			}
			...
			this.element.on("click", "button", function() {
				...
				switch (pageText) {
					...
					case "before":
						...
						break;
					case "after":
						settings.page = Number(settings.page) + Number(settings.fastForward) <= pageTatol ? Number(settings.page) +
							Number(settings.fastForward) : pageTatol;
						pageText = settings.page;
						break;
					case "go":
